...
#define _ss_addr_p(x) ((fFamily==AF_INET6) ? (void*)&(((struct sockaddr_in6*)&x)->sin6_addr) : (void*)&(((struct sockaddr_in*)&x)->sin_addr))
...
#define JNS_MCAST  0x10
    ...
    *  This will ensure proper workarounds for Multicast incompatibility will be activated while setting up address family.
    */
	...
	if(bind(fSockfd, (struct sockaddr*)(&fRecvAddr), sizeof(fRecvAddr))) return SOCKET_ERROR;
	fState |= JNS_BOUND;
	return 0;
    }
    ...
    */
    ...
    int JackNetUnixSocket::SendTo(const void* buffer, size_t nbytes, int flags)
    {
        ...
        if (WaitWrite() < 0)
            return -1;
    #endif
        return sendto(fSockfd, buffer, nbytes, flags, reinterpret_cast<sockaddr *>(&fSendAddr), sizeof(fSendAddr));
    }
    ...
    int JackNetUnixSocket::RecvFrom(void* buffer, size_t nbytes, int flags)
    {
        ...
        if (WaitRead() < 0)
            return -1;
    #endif
        return recvfrom(fSockfd, buffer, nbytes, flags, (struct sockaddr*)(&fRecvAddr), &addr_len);
    }
    ...
    int JackNetUnixSocket::CatchHost(void* buffer, size_t nbytes, int flags)
    {
        ...
        if (WaitRead() < 0)
            return -1;
    #endif
        return recvfrom(fSockfd, buffer, nbytes, flags, (struct sockaddr *)(&fSendAddr), &addr_len);
    }
